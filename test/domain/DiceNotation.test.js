...
assert.strictEqual(parsed.components.length, 2);assert.deepStrictEqual(parsed.components[0], {
    type: 'dice',
    count: 2,
    sides: 8,
    sign: 1
    ...
    it('should throw error for invalid dice counts', () => {assert.throws(() => DiceNotation.parse('0d6'), /Invalid dice count/);
});
...
assert(result.total <= 4, 'Total should be at most 4 (6-2)');
    ...
    for (let i = 0; i < 20; i++) {
    const total = DiceNotation.roll('1d4+2');assert(total >= 3, 'Total should be at least 3');
